...
import { UserAlertPreference, IUserAlertPreference, AlertState } from '../models/UserAlertPreference';
import { Database } from '../database/Database';
import { v4 as uuidv4 } from 'uuid';

export class UserAlertPreferenceRepository {
  private db: Database;
  ...
  async create(preference: IUserAlertPreference): Promise<UserAlertPreference> {
    const id = uuidv4();
  ...
  async update(id: string, updates: Partial<IUserAlertPreference>): Promise<UserAlertPreference | null> {
    ...
    return new UserAlertPreference({ ...updatedPref, updatedAt: new Date(now) });
  }

  async findById(id: string): Promise<UserAlertPreference | null> {
  ...
  async findByUserAndAlert(userId: string, alertId: string): Promise<UserAlertPreference | null> {
  ...
  async findByUser(userId: string): Promise<UserAlertPreference[]> {
    const sql = `
  ...
  async findByAlert(alertId: string): Promise<UserAlertPreference[]> {
    const sql = `
      SELECT * FROM user_alert_preferences 
      WHERE alert_id = ? 
      ORDER BY updated_at DESC
  ...
  async findUnreadByUser(userId: string): Promise<UserAlertPreference[]> {
    const sql = `
      SELECT * FROM user_alert_preferences 
      WHERE user_id = ? AND state = ?
      ORDER BY created_at DESC
    `;
    const rows = await this.db.all(sql, [userId, AlertState.UNREAD]);
    return rows.map(row => this.mapRowToPreference(row));
  }

  async findSnoozedByUser(userId: string): Promise<UserAlertPreference[]> {
    const sql = `
      SELECT * FROM user_alert_preferences 
      WHERE user_id = ? AND state = ?
      ORDER BY snoozed_until DESC
    `;
    const rows = await this.db.all(sql, [userId, AlertState.SNOOZED]);
    return rows.map(row => this.mapRowToPreference(row));
  }

  async getOrCreate(userId: string, alertId: string): Promise<UserAlertPreference> {
  ...
  async markAsRead(userId: string, alertId: string): Promise<UserAlertPreference | null> {
  ...
  async markAsUnread(userId: string, alertId: string): Promise<UserAlertPreference | null> {
    const preference = await this.getOrCreate(userId, alertId);
    preference.markAsUnread();
    return this.update(preference.id!, preference.toJSON());
  }

  async snoozeForDay(userId: string, alertId: string): Promise<UserAlertPreference | null> {
    const preference = await this.getOrCreate(userId, alertId);
    preference.snoozeForDay();
    return this.update(preference.id!, preference.toJSON());
  }

  async incrementNotificationCount(userId: string, alertId: string): Promise<UserAlertPreference | null> {
    const preference = await this.getOrCreate(userId, alertId);
  ...
  private mapRowToPreference(row: any): UserAlertPreference {
    return new UserAlertPreference({
      id: row.id,
      userId: row.user_id,
